...
// createTranslationVersioning creates a versioning record for a translation if versioning is enabled
func (m *MCP) createTranslationVersioning(ctx context.Context, translation cmsstore.TranslationInterface) error {
	if !m.store.VersioningEnabled() {
		return nil
	}
	...
	// Marshal translation content for versioning
	content, err := translation.Content()
	if err != nil {
		return err
	}

	translationData := map[string]any{
		"id":      translation.ID(),
		"name":    translation.Name(),
		"handle":  translation.Handle(),
		"content": content,
	...
	versioningContent, err := json.Marshal(translationData)
	if err != nil {
		return err
	}
	...
	if len(lastVersioningList) > 0 {
		...
		if lastVersioning.Content() == string(versioningContent) {
	...
	return m.store.VersioningCreate(ctx, cmsstore.NewVersioning().
		SetEntityID(translation.ID()).
		SetEntityType(cmsstore.VERSIONING_TYPE_TRANSLATION).
		SetContent(string(versioningContent)))
}
