...
type siteVersioningController struct {
	ui UiInterface
}

type siteVersioningControllerData struct {
...
func NewSiteVersioningController(ui UiInterface) *siteVersioningController {
	return &siteVersioningController{
		ui: ui,
	}
}

func (controller siteVersioningController) Handler(w http.ResponseWriter, r *http.Request) string {
...
func (controller *siteVersioningController) modal(data siteVersioningControllerData) hb.TagInterface {
	submitUrl := shared.URLR(data.request, shared.PathSitesSiteVersioning, map[string]string{
		"site_id":       data.siteID,
		"versioning_id": data.versioningID,
	})

	modalID := "ModalSiteVersioning"
	modalBackdropClass := "ModalBackdrop"

	modalCloseScript := `closeModal` + modalID + `();`

	modalHeading := hb.Heading5().HTML("Site Revisions").Style(`margin:0px;`)
	if data.versioning != nil {
		name := carbon.Parse(data.versioning.CreatedAt(), carbon.UTC).Format("Y-m-d H:i")
		modalHeading = hb.Heading5().HTML("Site Revision: " + name).Style(`margin:0px;`)
	}
	...
	jsCloseFn := `function closeModal` + modalID + `() {document.getElementById('ModalSiteVersioning').remove();[...document.getElementsByClassName('` + modalBackdropClass + `')].forEach(el => el.remove());}`
	...
	buttonSend := hb.Button().
		...
		HxInclude("#" + modalID).
		HxPost(submitUrl).
		HxSelectOob("#ModalSiteVersioning").
		HxTarget("body").
		HxSwap("beforeend")
...
func (controller *siteVersioningController) tableRevision(data siteVersioningControllerData) hb.TagInterface {
...
func (controller *siteVersioningController) tableRevisions(data siteVersioningControllerData) hb.TagInterface {
	return hb.Table().
		Class("table table-striped table-hover table-bordered").
		Children([]hb.TagInterface{
			hb.Thead().Children([]hb.TagInterface{
...
func (controller *siteVersioningController) prepareDataAndValidate(r *http.Request) (data siteVersioningControllerData, errorMessage string) {
	var err error
	data.request = r
	...
	site, err := controller.ui.Store().SiteFindByID(data.request.Context(), data.siteID)

	if err != nil {
		controller.ui.Logger().Error("At siteVersioningController > prepareDataAndValidate", "error", err.Error())
	...
	controller.restoreRevisionAttributes(data.request.Context(), site, data.versioning, attrs)
...
func (controller *siteVersioningController) restoreRevisionAttributes(ctx context.Context, site cmsstore.SiteInterface, versioning cmsstore.VersioningInterface, attrs []string) error {
	if site == nil {
	...
	for _, attr := range attrs {
		...
		if attr == cmsstore.COLUMN_STATUS {
			site.SetStatus(value)
		}

		if attr == cmsstore.COLUMN_DOMAIN_NAMES {
			domains := []string{}
	...
	err = controller.ui.Store().SiteUpdate(ctx, site)
...
func (controller *siteVersioningController) supportedAttributes() []string {
	return []string{
