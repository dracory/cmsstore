...
func TestDraftBlockTypeFieldIsEditable(t *testing.T) {
	...
	selectStr, _ := str.Between(body, `name="block_type"`, `</select>`)
	hasReadonlyText := strings.Contains(selectStr, `readonly`)
...
func TestDraftBlockCanChangeType(t *testing.T) {
	// Initialize test store
	store, err := testutils.InitStore(":memory:")
	if err != nil {
		t.Fatalf("InitStore should succeed, got error: %v", err)
...
func TestPublishedBlockCannotChangeType(t *testing.T) {
	// Initialize test store
	store, err := testutils.InitStore(":memory:")
	if err != nil {
		t.Fatalf("InitStore should succeed, got error: %v", err)
...
func TestPublishedBlockTypeFieldIsReadonly(t *testing.T) {
	// Initialize test store
	store, err := testutils.InitStore(":memory:")
	if err != nil {
		t.Fatalf("InitStore should succeed, got error: %v", err)
	...
	// Create a draft block first, then publish it
	draftBlock := cmsstore.NewBlock()
	draftBlock.SetName("Test Block")
	draftBlock.SetType(cmsstore.BLOCK_TYPE_MENU)
	draftBlock.SetStatus(cmsstore.BLOCK_STATUS_DRAFT)
	draftBlock.SetSiteID(site.ID())
	err = store.BlockCreate(context.Background(), draftBlock)
	if err != nil {
