...
type blockVersioningController struct {
	ui UiInterface
}

type blockVersioningControllerData struct {
...
func NewBlockVersioningController(ui UiInterface) *blockVersioningController {
	return &blockVersioningController{
		ui: ui,
	}
}

func (controller blockVersioningController) Handler(w http.ResponseWriter, r *http.Request) string {
...
func (controller *blockVersioningController) modal(data blockVersioningControllerData) hb.TagInterface {
	submitUrl := shared.URLR(data.request, shared.PathBlocksBlockVersioning, map[string]string{
		"block_id":      data.blockID,
		"versioning_id": data.versioningID,
	})

	modalID := "ModalBlockVersioning"
	...
	jsCloseFn := `function closeModal` + modalID + `() {document.getElementById('ModalBlockVersioning').remove();[...document.getElementsByClassName('` + modalBackdropClass + `')].forEach(el => el.remove());}`
	...
	buttonSend := hb.Button().
		...
		HxInclude("#" + modalID).
		HxPost(submitUrl).
		HxSelectOob("#ModalBlockVersioning").
		HxTarget("body").
		HxSwap("beforeend")
...
func (controller *blockVersioningController) tableRevision(data blockVersioningControllerData) hb.TagInterface {
...
func (controller *blockVersioningController) tableRevisions(data blockVersioningControllerData) hb.TagInterface {
	return hb.Table().
		Class("table table-striped table-hover table-bordered").
		Children([]hb.TagInterface{
			hb.Thead().Children([]hb.TagInterface{
...
func (controller *blockVersioningController) prepareDataAndValidate(r *http.Request) (data blockVersioningControllerData, errorMessage string) {
	var err error
	data.request = r
	data.blockID = strings.TrimSpace(req.GetStringTrimmed(r, "block_id"))
	...
	block, err := controller.ui.Store().BlockFindByID(data.request.Context(), data.blockID)
...
func (controller *blockVersioningController) restoreRevisionAttributes(ctx context.Context, block cmsstore.BlockInterface, versioning cmsstore.VersioningInterface, attrs []string) error {
	if block == nil {
	...
	err = controller.ui.Store().BlockUpdate(ctx, block)
...
func (controller *blockVersioningController) supportedAttributes() []string {
	return []string{
