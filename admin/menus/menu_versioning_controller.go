...
type menuVersioningController struct {
	ui UiInterface
}

type menuVersioningControllerData struct {
...
func NewMenuVersioningController(ui UiInterface) *menuVersioningController {
	return &menuVersioningController{
		ui: ui,
	}
}

func (controller menuVersioningController) Handler(w http.ResponseWriter, r *http.Request) string {
...
func (controller *menuVersioningController) modal(data menuVersioningControllerData) hb.TagInterface {
	submitUrl := shared.URLR(data.request, shared.PathMenusMenuVersioning, map[string]string{
		"menu_id":       data.menuID,
		"versioning_id": data.versioningID,
	})

	modalID := "ModalMenuVersioning"
	...
	jsCloseFn := `function closeModal` + modalID + `() {document.getElementById('ModalMenuVersioning').remove();[...document.getElementsByClassName('` + modalBackdropClass + `')].forEach(el => el.remove());}`
	...
	buttonSend := hb.Button().
		...
		HxInclude("#" + modalID).
		HxPost(submitUrl).
		HxSelectOob("#ModalMenuVersioning").
		HxTarget("body").
		HxSwap("beforeend")
...
func (controller *menuVersioningController) tableRevision(data menuVersioningControllerData) hb.TagInterface {
...
func (controller *menuVersioningController) tableRevisions(data menuVersioningControllerData) hb.TagInterface {
	return hb.Table().
		Class("table table-striped table-hover table-bordered").
		Children([]hb.TagInterface{
			hb.Thead().Children([]hb.TagInterface{
...
func (controller *menuVersioningController) prepareDataAndValidate(r *http.Request) (data menuVersioningControllerData, errorMessage string) {
	var err error
	data.request = r
	data.menuID = strings.TrimSpace(req.GetStringTrimmed(r, "menu_id"))
	...
	menu, err := controller.ui.Store().MenuFindByID(data.request.Context(), data.menuID)

	if err != nil {
		controller.ui.Logger().Error("At menuVersioningController > prepareDataAndValidate", "error", err.Error())
	...
	controller.restoreRevisionAttributes(data.request.Context(), menu, data.versioning, attrs)
...
func (controller *menuVersioningController) restoreRevisionAttributes(ctx context.Context, menu cmsstore.MenuInterface, versioning cmsstore.VersioningInterface, attrs []string) error {
	if menu == nil {
